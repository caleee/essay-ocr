...
class KRImageAdapter(val context: Context) : IKRImageAdapter {
    ...
    private fun requestImage(
        ...
        val requestBuilder = if (src.endsWith(".gif")) {
            Glide.with(EssayOCRApp.application)
                .asGif()
    ...
    private fun loadFromBase64(
        ...
        execOnSubThread {
            val srcParts = imageLoadOption.src.split(",")
            if (srcParts.size < 2) { callback.invoke(null); return@execOnSubThread }
            val bytes = Base64.decode(srcParts[1], Base64.DEFAULT)
            ...
            try {
                ...
                try {
                    options.inSampleSize = calculateInSampleSize(
                        options, imageLoadOption.requestWidth, imageLoadOption.requestHeight,
                    )
                } catch (e: ArithmeticException) {
                    Log.w("KRImageAdapter", "calculateInSampleSize failed", e)
                }
                val bitmap = BitmapFactory.decodeByteArray(bytes, 0, bytes.size, options)
                callback.invoke(BitmapDrawable(Resources.getSystem(), bitmap))
            } catch (e: OutOfMemoryError) {
                Log.e("KRImageAdapter", "OOM decoding base64 image", e)
            }
        }
    }
